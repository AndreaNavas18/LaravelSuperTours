...
return new class extends Migration
{
    ...
    public function up()
    {
        Schema::create('traffic_report', function (Blueprint $table) {
            $table->integer('id');
            $table->integer('id_tour')->nullable();
            $table->string('type_tour', 10);
            $table->time('time_am')->comment('Hora de recogida en la mañana');
            $table->string('from1', 100)->nullable()->comment('A donde lo tienen que ir a buscar');
            $table->string('to1', 100)->nullable()->comment('A donde tienen que dejarlo');
            $table->integer('id_attraction_trafic')->nullable();
            $table->date('date1')->comment('Fecha del trafico');
    ...
    {
        Schema::dropIfExists('traffic_report');
    }
};
