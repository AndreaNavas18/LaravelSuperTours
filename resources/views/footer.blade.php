<footer class="custom-footer-py footerMovil">
    <div class="backgroundimag">
        <div style="background-color: rgba(0, 0, 0, 0.699); height: 200px;">
            <section class=" wrapper-footer footer-area md:footer-area-md py-1 custom-footerr">
                <div class="flex gap-4 justify-start w-full md:w-[50%] footer1Div">
                    <a href="#" class="footer1Div1">
                        <img src="{{ asset('images/logo-white.png') }}" class="logoFooter1 logoFooterMovil1">
                        <img src="{{ asset('images/logosin.png') }}" class="logoFooter2">
                    </a>
                    <div class="flex gap-4 justify-start md:self-start footer2Div footer2DivMovil">
                        <h3 class="socialMediaFooter socialMediaFooterMovil">Follow Us</h3>
                        <a href="#">
                            <img src="{{ asset('images/facebook.png') }}" class="w-8">
                        </a>
                    </div>
                    <div class="footer1Div2 footer1DivMovil2">
                        <h3 class="titlesFooter titlesFooterMovil">Contact</h3>
                        <p class="text-dark-grayish-blue">Orlando (407)-370-3001</p>
                        <p class="text-dark-grayish-blue">Miami (305)-677-2676</p>
                        <h3 class="titlesFooter titlesFooterMovil">Email</h3>
                        <p class="text-dark-grayish-blue">reservations@supertours.com</p>
                    </div>
                </div>
                <div class="footer3Div footer3DivMovil">
                    <p class="text-center w-full md:w-auto md:col-span-2 md:self-center copyMovil copyPc">© 2023 Super Tours Inc. Copyright © 1989 - 2023 Super Tours Inc. All Rights Reserved.</p>
                </div>
            </section>
        </div>
    </div>
</footer>